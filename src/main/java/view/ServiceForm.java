...
public class ServiceForm {
	...
	public Service createService() {
		...
		Integer maxValue = SqliteJdbcTemplate.getJdbcTemplate().queryForObject(
				"select max(id) from service", int.class);
	...
	private Service createDialog(Dialog<Service> dialog, @Nullable Service editedService) {
		ButtonType buttonType = new ButtonType(editedService == null ? "Create" : "Ok",
				ButtonData.OK_DONE);
		...
		status = new ComboBox<String>(FXCollections.observableArrayList(ServiceStatus.listOfStatus
				.values()));
		...
		panel.add(price, 1, 8);
		Button addEnviroment = new Button("Add Enviroment");
		addEnviroment.setOnAction(e -> {
			
		});
		panel.add(addEnviroment, 2, 8);

		dialog.getDialogPane().setContent(panel);

		dialog.setResultConverter(btn -> {
			if (btn == buttonType) {
				return Service.builder().id(!id.getText().isEmpty() ? Integer.valueOf(id.getText()) : 0)
						.clientId(Integer.parseInt(clientId.getText())).serviceName(serviceName.getText())
						.dateOfOrder(dateOfOrder.getValue()).dateOfReceipt(dateOfReceipt.getValue())
						.info(info.getText())
						.serviceStatusId(status.getSelectionModel().getSelectedIndex() + 2)
						.assignedPersonId(assignedPersonId).assignedPersonValue(assignedPerson.getText())
		...
		if (service.isPresent()) {
			if (service.get().getDateOfReceipt() != null) {
				SqliteJdbcTemplate
						.getJdbcTemplate()
						.update(
								"insert into service_history(client_id,service_name,info,date_of_order,date_of_receipt,department_id,assigned_person_id,facture_id,price) values("
										+ service.get().getClientId()
										+ ", '"
										+ service.get().getName()
										+ "', '"
										+ service.get().getInfo()
										+ "', '"
										+ service.get().getDateOfOrder()
										+ "', '"
										+ service.get().getDateOfReceipt()
										+ "', "
										+ service.get().getDepartment()
										+ ", "
										+ service.get().getAssignedPersonId()
										+ ", "
										+ service.get().getInvoiceId()
										+ service.get().getPrice()+ ")");
				SqliteJdbcTemplate.getJdbcTemplate().update(
						"delete from service where id = " + service.get().getId());
				return new Service();
			}
			if ("Create".equals(buttonType.getText()))
				SqliteJdbcTemplate
						.getJdbcTemplate()
						.update(
								"insert into service(id, client_id, service_name, info, date_of_order, date_of_receipt, service_status_id, assigned_person_id, price) values ("
										+ service.get().getId()
										+ ", "
										+ service.get().getClientId()
										+ ", '"
										...
										+ service.get().getDateOfOrder()
										+ "', "
										+ (service.get().getDateOfReceipt() != null ? "'" + service.get().getDateOfReceipt() + "'" : null)
										+ ", "
										+ service.get().getserviceStatusId()
										+ ", "
										+ service.get().getAssignedPersonId()
										+ ", "
										+ service.get().getPrice()+ ")");
				...
				SqliteJdbcTemplate.getJdbcTemplate().update(
						"update service set client_id = " + service.get().getClientId() + ", service_name = '"
								+ service.get().getName() + "', info = '" + service.get().getInfo()
								+ "', date_of_order = '" + service.get().getDateOfOrder()
								+ "', date_of_receipt = '" + service.get().getDateOfReceipt()
								+ "', service_status_id = " + service.get().getserviceStatusId()
								+ ", assigned_person_id = " + service.get().getAssignedPersonId() 
								+ ", price = " + service.get().getPrice() + " where id = "
								+ service.get().getId());
			return service.get();
		}
		return null;
